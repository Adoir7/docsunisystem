<?php
	...
	class m_paineislib extends Model {

		public function __construct(){
			parent:: __construct();
		}

	   public function ch_portester() {
		  $sql = "SELECT   DISTINCT COUNT(SC.NR_CHAMADO) AS QUANTIDADE,
						 'ADOIR' AS TESTER    
					 FROM CRM_ANDAMENTOSAIDA_CQ SC
						  WHERE SC.ANALISTASAICQ LIKE '%LIBERADOR%'
						  AND SC.SAIDACQ = SC.ULTIMOANDAMENTO
						  AND SC.DETALHAMENTO_SAICQ LIKE '%ADOIR%'
					  GROUP BY SC.STATUS_SAICQ      
					 
				UNION 

				SELECT   DISTINCT COUNT(SC.NR_CHAMADO) AS QUANTIDADE,
						 'CLEBERSON' AS TESTER    
					 FROM CRM_ANDAMENTOSAIDA_CQ SC
						  WHERE SC.ANALISTASAICQ LIKE '%LIBERADOR%'
						  AND SC.SAIDACQ = SC.ULTIMOANDAMENTO
						  AND SC.DETALHAMENTO_SAICQ LIKE '%CLEBERSON%'
					  GROUP BY SC.STATUS_SAICQ      
				...
				SELECT  DISTINCT COUNT(SC.NR_CHAMADO) AS QUANTIDADE,
					...
					FROM CRM_ANDAMENTOSAIDA_CQ SC
						  WHERE SC.ANALISTASAICQ LIKE '%LIBERADOR%'
						  AND SC.SAIDACQ = SC.ULTIMOANDAMENTO
						  AND SC.DETALHAMENTO_SAICQ LIKE '%VITOR%'
					  GROUP BY SC.STATUS_SAICQ
					  ORDER BY 2 ";
		...
		public function lista_porstatus_liberador(){
			
			$sql = "SELECT DISTINCT LIB.STATUS_ATUAL,
				    COUNT(LIB.STATUS_ATUAL) AS QTDE
				   
						FROM (SELECT  CH.NR_CHAMADO,
							TRUNC(CH.DATA_CHAMADO) AS DT_CHAMADO,
								INITCAP(CL.NOME_CLIENTE) AS CLIENTE,
								INITCAP(SL.NOME_SOLICITANTE) AS SOLICITANTE,
					...
					REPLACE(OPCOES.NOME_OPCAO,'&','') as OPCAO, 
					  ...
					  '' AS ANDAMENTO,
					  '' AS DESTINO,
					fdescstatus(ca.status) as STATUS_ATUAL
					...
					FROM  crm_chamados ch, 
						  ...
						  MENUS,
						  MENUS_OPCOES OPCOES
					 WHERE ca.seq_pla_analista = '  47932601'
					 AND ce.seq_pla_cliente = cl.seq_pla_cliente
					 AND CH.COD_SISTEMA         = MENUS.COD_SISTEMA (+)
					 AND CH.COD_MENU            = MENUS.COD_MENU (+)
					 AND CH.COD_SISTEMA         = OPCOES.COD_SISTEMA (+)
					...
					FROM crm_andamento ca
					 WHERE ca.seq_pla_chamado = ch.seq_pla_chamado)
					 AND ch.situacao = 'P'
					 AND ch.seq_pla_ocorrencia = co.seq_pla_ocorrencia) LIB
					 WHERE 1=1
					 GROUP BY LIB.STATUS_ATUAL";
